...
# List Revo3 Modbus serial candidates (ttyACM / ttyUSB) with USB topology hints.

ROLES_FILE="${REVO3_USB_ROLES_FILE:-${XDG_CONFIG_HOME:-$HOME/.config}/revoarm/revo3_usb_roles.conf}"
...
echo "=== Revo3 serial discovery (Modbus) ==="
echo ""

mapfile -t DEVS < <(list_serial_devs | sort -V)
if [[ ${#DEVS[@]} -eq 0 ]]; then
  echo "No hardware serial ports found in /dev/. Plug in Revo3 USB cables." >&2
  exit 1
fi
...
if [[ -f "${ROLES_FILE}" ]]; then
  ...
  echo "  bootstrap: bash $(dirname "$0")/bootstrap_revo3.sh"
else
  echo "One-shot stable naming:"
  echo "  bash $(dirname "$0")/bootstrap_revo3.sh              # default: auto"
  echo "  bash $(dirname "$0")/bootstrap_revo3.sh --manual"
  echo "  bash $(dirname "$0")/bootstrap_revo3.sh /dev/ttyACM6 l /dev/ttyACM1 r"
  echo ""
  echo "Tip: unplug ONE hand — the device that disappears is on that hand's USB port."
fi

echo ""
echo "After udev: /dev/revo3_hand_left  /dev/revo3_hand_right"
echo "Done."
