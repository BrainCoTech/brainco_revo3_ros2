...
# Auto-generate stable udev rules for Revo3 Modbus serial (ttyACM/ttyUSB).
#
# Examples:
#   sudo bash setup_revo3_udev_rules.sh /dev/ttyACM6 l /dev/ttyACM1 r
#   sudo bash setup_revo3_udev_rules.sh /dev/ttyACM6 /dev/ttyACM1
#   sudo bash setup_revo3_udev_rules.sh /dev/ttyACM6 left
#
# Result symlinks:
#   /dev/revo3_hand_left
#   /dev/revo3_hand_right

RULE_FILE="/etc/udev/rules.d/99-revo3-hands.rules"
LEFT_NAME="revo3_hand_left"
RIGHT_NAME="revo3_hand_right"
...
main() {
  ...
  cat > "${RULE_FILE}" <<EOF
# Auto-generated by setup_revo3_udev_rules.sh
# Stable Modbus serial symlinks for Revo3 bimanual teleop.
EOF
  if [[ -n "${left_rule}" ]]; then
